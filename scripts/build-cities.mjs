...
const TARGET = resolve(ROOT, "src/data/cities.geonames.generated.ts");
...
function build() {
  ...
  lines_out.push(
    ...
    "// Run `npm run build:cities` to regenerate.",
    "//",
    "// Source: GeoNames cities15000 dump",
    "//   https://download.geonames.org/export/dump/cities15000.zip",
    "//   Licensed under Creative Commons Attribution 4.0 (CC BY 4.0)",
    "//   © GeoNames - https://www.geonames.org/",
    "//",
    "// Founding dates are NOT in GeoNames. Each entry's `founded` is a country-",
    "// level conservative default (see COUNTRY_FOUNDED_DEFAULT in the build",
    "// script). Cities with accurate founding dates are kept in cities.curated.ts",
    "// and take priority during the merge in cities.ts.",
    "",
    'import type { City } from "./cities.curated";',
    "",
    "export const GEONAMES_CITIES: readonly City[] = [",
  );
  for (const c of trimmed) {
    ...
    lines_out.push(
      ...
      `    cc: "${c.cc}",`,
      `    note: "${note}",`,
      `    populationCurve: [[${founded}, 5], [2025, ${Math.round(c.pop / 1000)}]],`,
    );
    if (c.capital) {
      lines_out.push(`    capital: true,`);
    }
    lines_out.push("  },");
  }
  lines_out.push("];", "");

  // Emit the curated-id → cc map. Consumed by src/data/cities.ts to back-fill
  // country codes on hand-curated entries that don't carry their own cc.
  const sortedCuratedIds = Object.keys(curatedCc).sort();
  lines_out.push(
    "/** Auto-derived ISO-2 country codes for hand-curated cities, looked up",
    " *  by nearest GeoNames row (within ~55 km). Curated entries without a",
    " *  match here fall back to nearest country centroid in the population",
    " *  layer. */",
    "export const CURATED_CITY_CC: Record<string, string> = {",
  );
  for (const id of sortedCuratedIds) {
    lines_out.push(`  "${id}": "${curatedCc[id]}",`);
  }
  lines_out.push("};", "");

  // Curated cities that overlap a modern national capital. Consumed by
  // src/data/cities.ts so curated entries like beijing / cairo / rome /
  // constantinople inherit the capital flag without us hand-marking them.
  const sortedCuratedCapitals = [...curatedCapitals].sort();
  lines_out.push(
    "/** Curated city ids that match a modern national capital (PPLC) by",
    " *  slug, name, or proximity. The merge in cities.ts uses this to set",
    " *  `capital: true` on curated entries that displace a GeoNames PPLC. */",
    "export const CURATED_CAPITAL_IDS: ReadonlySet<string> = new Set([",
  );
  for (const id of sortedCuratedCapitals) {
    lines_out.push(`  "${id}",`);
  }
  lines_out.push("]);", "");

  mkdirSync(dirname(TARGET), { recursive: true });
  writeFileSync(TARGET, lines_out.join("\n"));
