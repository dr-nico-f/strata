...
async function build() {
  ...
  const header = `// AUTO-GENERATED by scripts/build-disasters.mjs. DO NOT EDIT BY HAND.
// Run \`npm run build:disasters\` to regenerate.
//
// Sources:
//   USGS earthquake catalog (https://earthquake.usgs.gov/fdsnws/event/1/) -
//     M >= ${USGS_MIN_MAG} earthquakes ${USGS_START} - ${USGS_END}. Public domain.
//   Wikidata (https://query.wikidata.org/) -
//     volcanic eruptions, tsunamis, pandemics, famines, tropical cyclones,
//     and pre-1900 earthquakes. CC0.

import type { Disaster } from "./disasters.curated";

export const LIVE_DISASTERS: readonly Disaster[] = [
`;
  const body = disasters
    .map((d) => {
      const wpSlug = d.source === "Wikidata" ? d.name : "";
      return `  {
    id: "${escapeStringLiteral(d.id)}",
    name: "${escapeStringLiteral(d.name)}",
    year: ${d.year},
    kind: "${d.kind}",
    lat: ${d.lat},
    lng: ${d.lng},
    description: "${escapeStringLiteral(d.description)}",${
      wpSlug ? `\n    wikipedia: "${escapeStringLiteral(wpSlug)}",` : ""
    }${d.qid ? `\n    wikidata: "${d.qid}",` : ""}${d.magnitude !== undefined ? `\n    magnitude: ${d.magnitude},` : ""}
  },`;
    })
    .join("\n");
